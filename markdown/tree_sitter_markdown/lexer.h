...
public:
  ...
  LexedLength adv_rpt_len(const LexedCharacter character,
                          const LexedLength max_length = LEXED_LENGTH_MAX,
                          const bool skip = false);
  LexedLength adv_rpt_len(bool (*is_character)(LexedCharacter),
                          const LexedLength max_length = LEXED_LENGTH_MAX,
                          const bool skip = false);

  void bgn_buf();
  void jmp_pos(const LexedPosition &position);
  bool has_chr_at_idx(const LexedCharacter character,
                      const LexedIndex index) const;
  bool has_chr_at_idx(bool (*is_character)(LexedCharacter),
                      const LexedIndex index) const;
  bool has_chr_in_rng(bool (*is_character)(LexedCharacter),
                      const LexedIndex index, const LexedIndex end_index) const;
  bool has_chr_in_rng(bool (*is_character)(LexedCharacter),
                      const LexedPosition position,
                      const LexedPosition end_position) const;

  void bgn_rec_tbl_col_cnt();
  void end_rec_tbl_col_cnt();
  uint16_t tbl_col_cnt();

  LexedLength clc_vtr_spc_cnt(
      const LexedColumn current_indentation,
      const LexedColumn target_indentation,
      LexedLength &indentation_character_count_between_current_and_target)
      const;
